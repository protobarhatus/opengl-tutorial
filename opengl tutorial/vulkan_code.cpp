...
void VulkanApp::setMashScene(const MashInfo& scene)
{
    this->mash_scene = scene;
}
...
void VulkanApp::makeRenderPassForTriags()
{
    VkAttachmentDescription colorAttachment{};
    ...
    if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &triags_render_pass) != VK_SUCCESS) {
        throw std::runtime_error("failed to create render pass!");
    }
}
void VulkanApp::makeDescriptorSetLayoutForTriags()
{
    VkDescriptorSetLayoutBinding uboLayoutBinding{};
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &triag_ds_layout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create descriptor set layout!");
    }
}
void VulkanApp::makePipelineForTriags()
{
    auto vertShaderCode = readBinFile("triags_ren_vert.spv");
    auto fragShaderCode = readBinFile("triags_ren_frag.spv");
    ...
    auto vertex_binding_description = this->mash_scene.getVertexBindingDescription();
    auto attribute_binding_description = this->mash_scene.getAttributeDescriptions();
    vertexInputInfo.pVertexBindingDescriptions = &vertex_binding_description;
    vertexInputInfo.vertexAttributeDescriptionCount = attribute_binding_description.size();
    vertexInputInfo.pVertexAttributeDescriptions = attribute_binding_description.data();
    ...
    pipelineLayoutInfo.pSetLayouts = &triag_ds_layout;

    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &triagsPipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create pipeline layout!");
    }
    ...
    pipelineInfo.layout = triagsPipelineLayout;
    pipelineInfo.renderPass = triags_render_pass;
    pipelineInfo.subpass = 0;
    pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &triagsPipeline) != VK_SUCCESS) {
...
void VulkanApp::makeDescriptorSetAndPoolForTriags()
{
    std::array<VkDescriptorPoolSize, 1> poolSizes{};
    ...
    if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &triag_ds_pool) != VK_SUCCESS) {
        throw std::runtime_error("failed to create descriptor pool!");
    }
    ...
    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = triag_ds_pool;
    allocInfo.descriptorSetCount = static_cast<uint32_t>(1);
    allocInfo.pSetLayouts = &triag_ds_layout;

    
    if (vkAllocateDescriptorSets(device, &allocInfo, &triag_ds_set) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate descriptor sets!");
    }
    ...
    for (size_t i = 0; i < 1; i++) {
        ...
        descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites[0].dstSet = triag_ds_set;
        descriptorWrites[0].dstBinding = 0;
        descriptorWrites[0].dstArrayElement = 0;
...
void VulkanApp::makeBuffersForTriags()
{
    ...
    framebufferInfo.renderPass = triags_render_pass;
    ...
    if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &triag_frame_buffer) != VK_SUCCESS) {
        throw std::runtime_error("failed to create framebuffer!");
    }

    createBuffer(this->mash_scene.vertexes.size() * sizeof(Vertex), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertex_buffer, vertex_buffer_memory);
    createBuffer(this->mash_scene.indices.size() * sizeof(uint32_t), VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indices_buffer, indices_buffer_memory);
    copyDataToLocalBuffer(vertex_buffer, mash_scene.vertexes.data(), this->mash_scene.vertexes.size() * sizeof(Vertex));
    copyDataToLocalBuffer(indices_buffer, mash_scene.indices.data(), this->mash_scene.indices.size() * sizeof(uint32_t));
}



void VulkanApp::prepareTriagsPipeline()
{
    makeRenderPassForTriags();
    makeDescriptorSetLayoutForTriags();
    makePipelineForTriags();
    makeDescriptorSetAndPoolForTriags();
    makeBuffersForTriags();
...
void VulkanApp::updateUniformBuffer(uint32_t currentImage)
{
    ...
    auto view = glm::lookAt(glm::vec3(this->camera.x(), camera.y(), camera.z()), glm::vec3((camera + camera_direction).x(), (camera + camera_direction).y(), (camera + camera_direction).z()),
        glm::vec3(up_vector.x(), up_vector.y(), up_vector.z()));
    auto proj = glm::perspective(glm::radians(90.0f), (float)WIDTH / (float)HEIGHT, (float)z_near, (float)z_far);

    auto proj_view = proj * view;
    dat.prdat = { {(float)camera.x(), (float)camera.y(), (float)camera.z(), 0}, {(int)this->WIDTH, (int)this->HEIGHT}, scene.getPrimitivesCount(), scene.getDataCount(), scene.getNormalsCount(), scene.getComposedObjectNodesCount(), {0,0},
    ...
    dat.mat_dat.proj_view = proj_view;
    memcpy(uniformBuffersMapped[0], &dat, sizeof(dat));

}

void VulkanApp::drawTriags()
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();

    VkRenderPassBeginInfo renderPassInfo{};
    renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
    renderPassInfo.renderPass = triags_render_pass;
    renderPassInfo.framebuffer = triag_frame_buffer;
    renderPassInfo.renderArea.offset = { 0, 0 };
    renderPassInfo.renderArea.extent = swapChainExtent;
    ...
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, triagsPipeline);
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, triagsPipelineLayout, 0, 1, &triag_ds_set, 0, nullptr);
    VkDeviceSize offsets = 0;
    vkCmdBindVertexBuffers(commandBuffer, 0, 1, &vertex_buffer, &offsets);
    vkCmdBindIndexBuffer(commandBuffer, indices_buffer, 0, VK_INDEX_TYPE_UINT32);

    //vkCmdDraw(commandBuffer, 6, 1, 0, 0);
    vkCmdDrawIndexed(commandBuffer, mash_scene.indices.size(), 1, 0, 0, 0);
   ...
   /* VkSubmitInfo submitInfo{};
    ...
    if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS)
        throw std::runtime_error("failed to submit triag queue");
    vkWaitForFences(device, 1, &fence, VK_TRUE, 1000);
...
void VulkanApp::createTextureImage()
{
   ...
   // unsigned char* texture = stbi_load("terrain-atlas.tga", &width, &height, &bpp, 4);
    ...
    //VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT - its for renderpass for triags_pipeline
    createImage(WIDTH, HEIGHT, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
...
VkVertexInputBindingDescription VulkanApp::MashInfo::getVertexBindingDescription()
{
...
std::vector<VkVertexInputAttributeDescription> VulkanApp::MashInfo::getAttributeDescriptions()
{
    std::vector<VkVertexInputAttributeDescription> res(1);
    res[0].binding = 0;
    res[0].location = 0;
